...
$OutputDocx = Join-Path $ExportsDir "MEMORIA_TECNICA_MQTT_ROUTER.docx"
...
if ($mmdcCommand) {
    ...
    Add-ReportLine $report "- Instalación manual sugerida:"
    Add-ReportLine $report ""
...
if ($rsvgCommand) {
    ...
    Add-ReportLine $report "- Imágenes para Pandoc: SVG"
} else {
    Add-ReportLine $report "- rsvg-convert: NO ENCONTRADO"
    Add-ReportLine $report "- Imágenes para Pandoc: PNG generado por Mermaid CLI como fallback; los SVG se siguen generando."
}
Add-ReportLine $report ""
...
if (-not $pandocCommand) {
    ...
    Set-Content -LiteralPath $ReportPath -Value $report -Encoding UTF8
    throw "Pandoc no está disponible en PATH."
}
...
if (-not $mmdcCommand -and $matches.Count -gt 0) {
    ...
    Add-ReportLine $report ""
    Add-ReportLine $report "- DOCX generado: False"
    Add-ReportLine $report "- Motivo: Mermaid CLI no está disponible."
    Set-Content -LiteralPath $ReportPath -Value $report -Encoding UTF8
    throw "Mermaid CLI (mmdc) no está disponible en PATH. Instalar con: npm install -g @mermaid-js/mermaid-cli"
}
...
$buildContent = $regex.Replace($source, {
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) {
            throw "mmdc SVG devolvió código $LASTEXITCODE"
        }
        ...
        if (-not $rsvgCommand) {
            ...
            if ($LASTEXITCODE -ne 0) {
                throw "mmdc PNG fallback devolvió código $LASTEXITCODE"
            }
...
$pandocArgs = @(
    ...
    "-o",
    "exports/MEMORIA_TECNICA_MQTT_ROUTER.docx",
    "--toc",
    "--toc-depth=2",
    "--number-sections",
...
if (Test-Path $ReferenceDocx) {
    ...
    Add-ReportLine $report "- reference.docx: no existe; se continúa con estilo por defecto."
}
...
if ($pandocExit -ne 0) {
    Add-ReportLine $report "- Pandoc: ERROR código $pandocExit"
    Add-ReportLine $report ""
    Add-ReportLine $report "## Resultado"
    Add-ReportLine $report ""
    Add-ReportLine $report "DOCX no generado."
    Set-Content -LiteralPath $ReportPath -Value $report -Encoding UTF8
    throw "Pandoc falló con código $pandocExit"
}
...
if ($docxExists) {
    ...
    Add-ReportLine $report "- DOCX tamaño bytes: $($docxItem.Length)"
}
